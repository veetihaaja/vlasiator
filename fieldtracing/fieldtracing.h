...
// Get the (integer valued) local fsgrid cell index (i,j,k) for the magnetic-field traced mapping point that node n is
// associated with This includes indices beyond local size (positive and negative) as we need to access ghost cells
template <class T>
std::array<fsgrid::FsIndex_t, 3> getLocalFsGridCellIndexWithGhostsForCoord(T& grid, const std::array<Real, 3>& x) {
   ...
   std::array<fsgrid::FsIndex_t, 3> retval = {(fsgrid::FsIndex_t)globalInd[0] - localStart[0],
                                              (fsgrid::FsIndex_t)globalInd[1] - localStart[1],
                                              (fsgrid::FsIndex_t)globalInd[2] - localStart[2]};
   return retval;
}
...
inline int ijk2Index(int i, int j, int k, std::array<int, 3> dims) { return i + j * dims[0] + k * dims[0] * dims[1]; }
...
bool traceFullFieldFunction(fsgrids::perbspan perb,
   ...
   if (!alongB) { // In this function, outwards indicates whether we trace along (true) or against (false) the field
                  // direction
      b[0] *= -1;
      b[1] *= -1;
      b[2] *= -1;
...
void richardsonExtrapolation(int i, std::vector<REAL>& table, REAL& maxError, std::array<int, 3> dims) {
   ...
   for (int dim = 0; dim < 3; dim++) {
      for (k = 1; k < i + 1; k++) {
         ...
         table.at(ijk2Index(i, k, dim, dims)) =
             table.at(ijk2Index(i, k - 1, dim, dims)) +
             (table.at(ijk2Index(i, k - 1, dim, dims)) - table.at(ijk2Index(i - 1, k - 1, dim, dims))) /
                 (std::pow(4, i) - 1);
      }

      REAL thisError =
          fabs(table.at(ijk2Index(k - 1, k - 1, dim, dims)) - table.at(ijk2Index(k - 2, k - 2, dim, dims)));
...
bool adaptiveEulerStep(std::array<REAL, 3>& r, std::array<REAL, 3>& b, REAL& stepSize, const REAL minStepSize,
   ...
   std::array<REAL, 3> error_xyz{fabs(r2[0] - r1[0]), fabs(r2[1] - r1[1]), fabs(r2[2] - r1[2])};
   ...
   if (err <= fieldTracingParameters.max_allowed_error || stepSize == minStepSize) {
      // Note: B at r has been evaluated above so no need to do it here and we're not returning it anyway as it's not
      // needed.
      r = r2;
      return true;
   } else {
...
void stepFieldLine(std::array<REAL, 3>& x, std::array<REAL, 3>& v, REAL& stepsize, const REAL minStepSize,
   ...
   case ADPT_Euler:
      ...
      if (reTrace) {
         logFile << "(fieldtracing) Warning: Adaptive Euler field line tracer exhausted all available attempts and "
   ...
   case DPrince:
      ...
      if (reTrace) {
         logFile << "(fieldtracing) Warning: Dormand Prince field line tracer exhausted all available attempts and "
                    "still did not converge..."
                 << std::endl;
      }
      break;
