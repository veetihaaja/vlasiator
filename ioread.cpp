 ...
 * @param mpiGrid Parallel grid library.
 * @return If true, velocity block data was read successfully.*/
bool readBlockData(vlsv::ParallelReader& file, const string& meshName, const vector<CellID>& fileCells,
                   const uint64_t localCellStartOffset, const uint64_t localCells,
                   dccrg::Dccrg<SpatialCell, dccrg::Cartesian_Geometry>& mpiGrid) {
   bool success = true;
   ...
   for (uint popID = 0; popID < getObjectWrapper().particleSpecies.size(); ++popID) {
      ...
      if (fileMeshBBox[0] != ourMeshParams.gridLength[0] ||
         ...
         for (const auto& c : fileVelCoordsX) {
            ...
            if (fabs(nearbyint(cellindex) - cellindex) > 1. / 10000.) {
               logFile << "(RESTART) ERROR: Can't resize velocity space as cell coordinates don't match." << endl
                       << "          (X coordinate " << c << " = " << cellindex << " * " << dVx << " + "
                       << vmesh::getMeshWrapper()->velocityMeshes->at(meshID).meshMinLimits[0] << endl
         ...
         for (const auto& c : fileVelCoordsY) {
            ...
            if (fabs(nearbyint(cellindex) - cellindex) > 1. / 10000.) {
               logFile << "(RESTART) ERROR: Can't resize velocity space as cell coordinates don't match." << endl
                       << "           (Y coordinate " << c << " = " << cellindex << " * " << dVy << " + "
                       << vmesh::getMeshWrapper()->velocityMeshes->at(meshID).meshMinLimits[1] << endl
                       << "           coordinate  = cellindex *   dV  +  meshMinLimits)" << endl
                       << write;
               abort();
            }
         ...
         for (const auto& c : fileVelCoordsY) {
            ...
            if (fabs(nearbyint(cellindex) - cellindex) > 1. / 10000.) {
               logFile << "(RESTART) ERROR: Can't resize velocity space as cell coordinates don't match." << endl
                       << "           (Z coordinate " << c << " = " << cellindex << " * " << dVz << " + "
                       << vmesh::getMeshWrapper()->velocityMeshes->at(meshID).meshMinLimits[2] << endl
                       << "           coordinate  = cellindex *   dV  +  meshMinLimits)" << endl
                       << write;
               abort();
            }
...
/*! Reads cell parameters from the file and saves them in the right place in mpiGrid
 ...
 \param localCellStartOffset Offset in the fileCells list for this process ( calculated so that the amount of blocks is
 distributed somewhat evenly between processes) \param localCells The amount of cells to read in this process after
 localCellStartOffset \param cellParamsIndex The parameter of the cell index e.g. CellParams::RHOM \param
 expectedVectorSize The amount of elements in the parameter (parameter can be a scalar or a vector of size N) \param
 mpiGrid Vlasiator's grid (the parameters are saved here) \return Returns true if the operation is successful
 */
template <typename fileReal>
static bool _readCellParamsVariable(vlsv::ParallelReader& file, const vector<uint64_t>& fileCells,
                                    const uint64_t localCellStartOffset, const uint64_t localCells,
                                    const string& variableName, const size_t cellParamsIndex,
...
/*! Reads cell parameters from the file and saves them in the right place in mpiGrid
 ...
 mpiGrid Vlasiator's grid (the parameters are saved here) \return Returns true if the operation is successful
 */
bool readCellParamsVariable(vlsv::ParallelReader& file, const vector<CellID>& fileCells,
                            const uint64_t localCellStartOffset, const uint64_t localCells, const string& variableName,
                            const size_t cellParamsIndex, const size_t expectedVectorSize,
                            dccrg::Dccrg<SpatialCell, dccrg::Cartesian_Geometry>& mpiGrid) {
   uint64_t arraySize;
   uint64_t vectorSize;
   vlsv::datatype::type dataType;
 ...
 * \param numWritingRanks Number of mpi ranks that were used to write this file (used for reconstruction of the spatial
 * order) \param targetGrid target location where the data will be stored.
 */
...
bool readFsGridVariable(vlsv::ParallelReader& file, const string& variableName, int numWritingRanks,
   ...
   if (!(dataType == vlsv::datatype::type::FLOAT && byteSize == sizeof(Real))) {
      logFile << "(RESTART) Converting floating point format of fsgrid variable " << variableName << " from "
              << byteSize * 8 << " bits to " << sizeof(Real) * 8 << " bits." << endl
              << write;
      convertFloatType = true;
      // Note: this implicitly assumes that Real is of type double, and we either read a double in directly, or read a
      // float and convert it to double.
   }
   getArrayInfo.stop();
   ...
   if (P::overrideReadFsGridDecomposition == fileDecomposition) {
      ...
      if (fileDecomposition[0] * fileDecomposition[1] * fileDecomposition[2] != fsgridInputRanks) {
         exitOnError(
             false,
             "(RESTART) Trying to use a manual FsGrid decomposition for a file with a differing number of input ranks.",
             MPI_COMM_WORLD);
      }
   }
   ...
   if (decomposition == fileDecomposition) {
      ...
      for (int task = 0; task < numWritingRanks; task++) {
         ...
         if (overlapSize[0] * overlapSize[1] * overlapSize[2] > 0) {
            ...
            if (!convertFloatType) {
               if (file.addMultireadUnit((char*)buffer.data(),
                                         thatTasksSize[0] * thatTasksSize[1] * thatTasksSize[2]) == false) {
                  logFile << "(RESTART)  ERROR: Failed to read fsgrid variable " << variableName << endl << write;
                  return false;
               }
               ...
               if (file.addMultireadUnit((char*)readBuffer.data(),
                                         thatTasksSize[0] * thatTasksSize[1] * thatTasksSize[2]) == false) {
                  logFile << "(RESTART)  ERROR: Failed to read fsgrid variable " << variableName << endl << write;
                  return false;
               }
            ...
            for (auto z = overlapStart[2]; z < overlapEnd[2]; z++) {
               for (auto y = overlapStart[1]; y < overlapEnd[1]; y++) {
                  for (auto x = overlapStart[0]; x < overlapEnd[0]; x++) {
                     ...
                     memcpy(targetData[stencil.indexFromOffset(-localStart[0], -localStart[1], -localStart[2])].data(),
...
bool readIonosphereNodeVariable(vlsv::ParallelReader& file, const string& variableName, SBC::SphericalTriGrid& grid,
   ...
   if (vectorSize != 1) {
      logFile << "(RESTART) ERROR: Trying to read vector valued (" << vectorSize
   ...
   if (grid.nodes.size() != arraySize) {
      logFile << "(RESTART) ERROR: Ionosphere restart size mismatch: trying to read variable " << variableName
              << " with " << arraySize << " values into a ionosphere grid with " << grid.nodes.size() << " nodes!"
              << endl
              << write;
      return false;
   }
...
bool exec_readGrid(dccrg::Dccrg<SpatialCell, dccrg::Cartesian_Geometry>& mpiGrid,
   ...
   // Backround B has to be set, there are also the derivatives that should be written/read if we wanted to only read in
   // background field
