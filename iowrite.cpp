...
bool writeVelocityDistributionData(const uint popID, Writer& vlsvWriter,
   ...
   for (int timeclass = 0; timeclass <= P::currentMaxTimeclass; timeclass++) {
      ...
      attribs["name"] = popName + "_tc_" + std::to_string(timeclass);

      // Compute totalBlocks
      uint64_t totalBlocks = 0;
      vector<vmesh::LocalID> blocksPerCell;
      for (size_t i = 0; i < cells.size(); ++i) {
         totalBlocks += mpiGrid[cells[i]]->get_number_of_velocity_blocks(popID, timeclass);
         blocksPerCell.push_back(mpiGrid[cells[i]]->get_number_of_velocity_blocks(popID, timeclass));
      }
      ...
      attribs.clear();
      attribs["mesh"] = getObjectWrapper().particleSpecies[popID].name + "_tc_" + std::to_string(timeclass);
      ...
      attribs["name"] = popName + "_tc_" + std::to_string(timeclass);      // Name of the velocity space distribution is written avgs
...
bool writeVspaceDataCompressionNone(const uint popID,const int timeclass,Writer& vlsvWriter,
                                   dccrg::Dccrg<SpatialCell,dccrg::Cartesian_Geometry>& mpiGrid,
   ...
   bool success=true;
   const string popName      = getObjectWrapper().particleSpecies[popID].name + "_tc_" + std::to_string(timeclass);
   const string spatMeshName = "SpatialGrid";
   map<string,string> attribs;
   ...
   try {
      ...
      for (size_t cell=0; cell<cells.size(); ++cell) {
         ...
         for (vmesh::LocalID block_i=0; block_i<SC->get_number_of_velocity_blocks(popID, timeclass); ++block_i) {
            vmesh::GlobalID block = SC->get_velocity_block_global_id(block_i,popID,timeclass);
...
#ifdef ASTERIX_ZFP
bool writeVspaceDataCompressionZFP(const uint popID, const int timeclass, Writer& vlsvWriter,
                                   dccrg::Dccrg<SpatialCell,dccrg::Cartesian_Geometry>& mpiGrid,
                                   const std::vector<CellID>& cells,std::size_t totalBlocks, MPI_Comm comm){
   ...
   for (const auto& cid:cells){
      totalElements+=mpiGrid[cid]->get_population(popID, timeclass).compressed_state_buffer.size();
   }
   ...
   for (size_t cell = 0; cell<cells.size(); ++cell) {
      ...
      // Get the number of blocks in this cell
      const uint64_t arrayElements = SC->get_population(popID, timeclass).compressed_state_buffer.size();
      char* arrayToWrite = reinterpret_cast<char*>(SC->get_population(popID, timeclass).compressed_state_buffer.data());
...
bool writeVspaceDataCompressionOCTREE(const uint popID, const int timeclass, Writer& vlsvWriter,
                                   dccrg::Dccrg<SpatialCell,dccrg::Cartesian_Geometry>& mpiGrid,
                                   const std::vector<CellID>& cells,std::size_t totalBlocks, MPI_Comm comm){
...
#ifdef ASTERIX_MLP
bool writeVspaceDataCompressionMLP(const uint popID, const int timeclass, Writer& vlsvWriter,
                                   dccrg::Dccrg<SpatialCell,dccrg::Cartesian_Geometry>& mpiGrid,
...
bool writeVelocityDistributionDataAsterix(const uint popID,Writer& vlsvWriter,
   ...
   for (int timeclass = 0; timeclass <= P::currentMaxTimeclass; timeclass++) {
      ...
      for (size_t cell=0; cell<cells.size(); ++cell){
         totalBlocks+=mpiGrid[cells[cell]]->get_number_of_velocity_blocks(popID, timeclass);
         blocksPerCell.push_back(mpiGrid[cells[cell]]->get_number_of_velocity_blocks(popID, timeclass));
         bytesPerCell.push_back(mpiGrid[cells[cell]]->get_population(popID, timeclass).compressed_state_buffer.size());
      }
      ...
      switch (P::vdf_compression_method){
         case P::ASTERIX_COMPRESSION_METHODS::NONE:
            success=writeVspaceDataCompressionNone(popID,timeclass,vlsvWriter,mpiGrid,cells,totalBlocks,comm);
            break;
   #ifdef ASTERIX_MLP
         case P::ASTERIX_COMPRESSION_METHODS::MLP:
            success=writeVspaceDataCompressionMLP(popID,timeclass,vlsvWriter,mpiGrid,cells,bytes,totalBlocks,comm);
            break;
         ...
         case P::ASTERIX_COMPRESSION_METHODS::ZFP:
            success=writeVspaceDataCompressionZFP(popID,timeclass,vlsvWriter,mpiGrid,cells,totalBlocks,comm);
            break;
   #endif
   #ifdef ASTERIX_OCTREE
         case P::ASTERIX_COMPRESSION_METHODS::OCTREE:
            success=writeVspaceDataCompressionOCTREE(popID,timeclass,vlsvWriter,mpiGrid,cells,totalBlocks,comm);
            break;
   #endif
         default:
...
bool writeDomainExtents(Writer& vlsvWriter, const string& meshName, const std::vector<CellID>& local_cells,
   ...
   if (vlsvWriter.writeArray("MESH_DOMAIN_EXTENTS", xmlAttributes, arraySize, vectorSize, ret) == false) {
      ...
      logFile << "(MAIN) writeGrid: ERROR FAILED TO WRITE MESH_DOMAIN_EXTENTS AT: " << __FILE__ << " " << __LINE__ << endl << writeVerbose;
      return false;
   }
...
/*! Writes the zone global id numbers into the file. The vlsv file needs to
 * know in which order the local cells + ghost cells are written. Local cells
 * are first appended to a vector called global ids, after which the ghost
 * cells are appended. The global ids vector will then be saved into a vlsv
 * file
 * \param mpiGrid Vlasiator's MPI grid
 * \param vlsvWriter Some vlsv writer with a file open
...
/*! Writes the node coordinates. This means basically every cell's node (Corner
 * of each cell). Note: The grid is a structured one, so writing the nodes
 * means starting from the corner of the grid and writing coordinates per every
 * cell length until reaching the other corner of the grid
 * \param vlsvWriter Some vlsv writer with a file open
...
bool writeFsGridMetadata(FieldSolverGrid& fsgrid, fsgrids::consttechnicalspan technical, vlsv::Writer& vlsvWriter, bool writeIDs = false) {
   ...
   if (writeIDs) {
      ...
      std::vector<fsgrid::FsSize_t> globalIds(static_cast<fsgrid::FsSize_t>(localSize[0] * localSize[1] * localSize[2]));
      // Should work in parallel too
      fsgrid.parallel_for([](int timerId) -> phiprof::Timer { return phiprof::Timer{timerId}; }, phiprof::initializeTimer("Map Refinement Level to FsGrid"), technical,
                          [=, &globalIds](const fsgrid::Coordinates& coordinates, const fsgrid::FsStencil& stencil, cuint sysBoundaryFlag, cuint sysBoundaryLayer) {
                             ...
                             globalIds[index] = globalIndex[2] * globalSize[0] * globalSize[1] + globalIndex[1] * globalSize[0] + globalIndex[0];
                          });
...
bool writeIonosphereGridMetadata(vlsv::Writer& vlsvWriter) {
   ...
   std::array<int64_t, 6> boundaryBox({1, 1, 1, 1, 1, 1});
...
bool writeVelocitySpace(dccrg::Dccrg<SpatialCell, dccrg::Cartesian_Geometry>& mpiGrid,
   ...
   for (uint i = 0; i < cells.size(); i++) {
      ...
      if (P::systemWriteDistributionWriteStride[index] > 0 && cells[i] % P::systemWriteDistributionWriteStride[index] == 0) {
      ...
      for (uint ishell = 0; ishell < P::systemWriteDistributionWriteShellRadius.size(); ishell++) {
         ...
         if (withinshell) {
            ...
            if ((P::xcells_ini == 1) || (P::ycells_ini == 1) || (P::zcells_ini == 1)) {
               // 1D or 2D
               if ((D2 >= D - 0.5 * DX) && (D2 < D + 0.5 * DX) && (T2 >= T - 0.5 * DX) && (T2 < T + 0.5 * DX))
                  stridecheck = true;
               // Special case for corners:
               if ((abs(D - T) < 0.5 * DX) && (dist2 > dist))
                  stridecheck = true;

               // Only save 1 cell touching axes
               if ((P::ycells_ini == 1) && (((cellX > -1.1 * DX) && (cellX < 0)) || ((cellZ > -1.1 * DZ) && (cellZ < 0))))
                  stridecheck = false;
               if ((P::zcells_ini == 1) && (((cellX > -1.1 * DX) && (cellX < 0)) || ((cellY > -1.1 * DY) && (cellY < 0))))
                  stridecheck = false;
               ...
               if ((s[1] < DX) && (s[0] < DX) && (D2 >= D - 0.5 * DX) && (D2 < D + 0.5 * DX))
                  stridecheck = true;

               // Special corner-corner-case
               if ((abs(s[2] - s[1]) < DX) && (abs(s[1] - s[0]) < DX) && (abs(s[2] - s[0]) < DX))
                  stridecheck = true;

               // Only save 1 cell touching axes (assumes origin is at corner intersection of 8 cells)
               if (((cellX > -1.1 * DX) && (cellX < 0)) || ((cellY > -1.1 * DY) && (cellY < 0)) || ((cellZ > -1.1 * DZ) && (cellZ < 0)))
                  stridecheck = false;
            }
...
bool writeGrid(
   ...
   if (dataReducer != NULL)
      for (uint i = 0; i < dataReducer->size(); ++i) {
         if (writeDataReducer(mpiGrid, local_cells, fieldSolverData, (P::writeAsFloat == 1), P::systemWriteFsGrid.at(outputFileTypeIndex), *dataReducer, i, vlsvWriter) == false) {
            return false;
         }
      }
   writeDataTimer.stop();
